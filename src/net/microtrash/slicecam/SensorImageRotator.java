...
public class SensorImageRotator {
	...
	public SensorImageRotator(Activity activity) {
		...
		try {
			defaultOrientation = display.getRotation(); // take care: this is
														// only working on API
														// level 8 and above. 0
														// => Landskape
														// (tablet), 1 =>
		...
		orientationEventListener = new OrientationEventListener(activity, SensorManager.SENSOR_DELAY_NORMAL) {
			...
			public void onOrientationChanged(int orientationValue) {
				...
				degreesQuantized = quantizeDegrees(orientationValue);
	...
	public static int quantizeDegrees(int orientationValue) {
		int degreesQuantized = 0; // default -> landscape

		if (orientationValue >= 315 || orientationValue < 45) { // portrait
			degreesQuantized = 90;
		} else if (orientationValue < 135 && orientationValue > 45) { // inversed
																		// landscape
			degreesQuantized = 180;
		} else if (orientationValue >= 135 && orientationValue < 230) { // inversed
																		// portrait
